...
```
```sh
$ dotenvx run --debug -- node index.mjs
...
$ echo "console.log('Hello ' + process.env.HELLO)" > index.mjs

$ dotenvx run --env-file=.env.production -- node index.mjs
Hello production
> ^^
```
...
$ dotenvx run --env-file=.env.local --env-file=.env -- node index.mjs
Hello local
```
